...
package com.netty100.cluster.naming.remote.udp;
...
import com.netty100.cluster.naming.push.v2.NoRequiredRetryException;
...
public class UdpConnector {
     ...
     * @throws CapException cap exception during sending
     */
    public void sendData(AckEntry ackEntry) throws CapException {
        if (null == ackEntry) {
            return;
        }
        try {
...
//            MetricsMonitor.incrementFailPush();
            throw new CapException(CapException.SERVER_ERROR, "[CAP-PUSH] push data with exception: ", e);
        }
    }
        ...
        public void run() {
            while (running) {
                ...
                try {
                    ...
                    Loggers.PUSH.error("[CAP-PUSH] error while receiving ack data", e);
                }
            }
        }
