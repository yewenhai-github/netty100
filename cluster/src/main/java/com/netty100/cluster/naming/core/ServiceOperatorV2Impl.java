...
import com.netty100.cluster.naming.constants.FieldsConstants;
import com.netty100.cluster.naming.core.v2.ServiceManager;
import com.netty100.cluster.naming.core.v2.index.ServiceStorage;
import com.netty100.cluster.naming.core.v2.metadata.ClusterMetadata;
...
import com.netty100.cluster.naming.pojo.ClusterInfo;
...
public class ServiceOperatorV2Impl implements ServiceOperator {
    ...
    @Override
    public void create(String namespaceId, String serviceName, ServiceMetadata metadata) throws CapException {
     ...
     */
    public void create(Service service, ServiceMetadata metadata) throws CapException {
        if (ServiceManager.getInstance().containSingleton(service)) {
            throw new CapApiException(CapException.INVALID_PARAM, ErrorCode.SERVICE_ALREADY_EXIST,
    ...
    @Override
    public void update(Service service, ServiceMetadata metadata) throws CapException {
        if (!ServiceManager.getInstance().containSingleton(service)) {
            throw new CapApiException(CapException.INVALID_PARAM, ErrorCode.SERVICE_NOT_EXIST,
    ...
    @Override
    public void delete(String namespaceId, String serviceName) throws CapException {
     ...
     */
    public void delete(Service service) throws CapException {
        if (!ServiceManager.getInstance().containSingleton(service)) {
            throw new CapApiException(CapException.INVALID_PARAM, ErrorCode.SERVICE_NOT_EXIST,
                    String.format("service %s not found!", service.getGroupedServiceName()));
        }
        ...
        if (!serviceStorage.getPushData(service).getHosts().isEmpty()) {
            throw new CapApiException(CapException.INVALID_PARAM, ErrorCode.SERVICE_DELETE_FAILURE,
    ...
    @Override
    public ObjectNode queryService(String namespaceId, String serviceName) throws CapException {
        Service service = getServiceFromGroupedServiceName(namespaceId, serviceName, true);
        if (!ServiceManager.getInstance().containSingleton(service)) {
            throw new CapApiException(CapException.INVALID_PARAM, ErrorCode.SERVICE_NOT_EXIST,
     ...
     */
    public ServiceDetailInfo queryService(Service service) throws CapException {
        if (!ServiceManager.getInstance().containSingleton(service)) {
            throw new CapApiException(CapException.INVALID_PARAM, ErrorCode.SERVICE_NOT_EXIST,
    ...
    public Collection<String> listService(String namespaceId, String groupName, String selector) throws CapException {
    ...
    @Override
    public Collection<String> searchServiceName(String namespaceId, String expr) throws CapException {
