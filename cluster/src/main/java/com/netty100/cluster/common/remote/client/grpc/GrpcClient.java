...
package com.netty100.cluster.common.remote.client.grpc;

import com.netty100.cluster.api.exception.CapException;
import com.netty100.cluster.api.grpc.auto.BiRequestStreamGrpc;
import com.netty100.cluster.api.grpc.auto.Payload;
import com.netty100.cluster.api.grpc.auto.RequestGrpc;
import com.netty100.cluster.api.remote.request.ConnectionSetupRequest;
import com.netty100.cluster.api.remote.request.Request;
import com.netty100.cluster.api.remote.request.ServerCheckRequest;
import com.netty100.cluster.api.remote.response.ErrorResponse;
import com.netty100.cluster.api.remote.response.Response;
import com.netty100.cluster.api.remote.response.ServerCheckResponse;
import com.netty100.cluster.common.packagescan.resource.Resource;
import com.netty100.cluster.common.remote.ConnectionType;
import com.netty100.cluster.common.remote.client.RpcClient;
import com.netty100.cluster.common.remote.client.RpcClientStatus;
import com.netty100.cluster.common.remote.client.Connection;
import com.netty100.cluster.common.remote.client.RpcClientTlsConfig;
...
import com.netty100.cluster.common.utils.TlsTypeResolve;
...
public abstract class GrpcClient extends RpcClient {
    ...
    protected ThreadPoolExecutor createGrpcExecutor(String serverIp) {
        ...
        ThreadPoolExecutor grpcExecutor = new ThreadPoolExecutor(clientConfig.threadPoolCoreSize(),
                ...
                new ThreadFactoryBuilder().daemon(true).nameFormat("cap-grpc-client-executor-" + serverIp + "-%d")
                        .build());
...
//            LoggerUtils.printIfErrorEnabled(LOGGER, "Server check fail, please check server {} ,port {} is available , error ={}", ip, port, e);
            LoggerUtils.printIfWarnEnabled(LOGGER, "Server check fail, please check server {} ,port {} is available , error ={}", ip, port, e.getMessage());
            return null;
        }
    }
                ...
                try {
                    ...
                    if (request != null) {
                        ...
                        try {
                            ...
                            Response errResponse = ErrorResponse.build(CapException.CLIENT_ERROR,
    ...
    private ManagedChannelBuilder buildChannel(String serverIp, int port, Optional<SslContext> sslContext) {
        if (sslContext.isPresent()) {
            NettyChannelBuilder r = NettyChannelBuilder.forAddress(serverIp, port)
                    .negotiationType(NegotiationType.TLS)
                    .sslContext(sslContext.get());
            return (ManagedChannelBuilder)r;
        } else {
            //明文连接
