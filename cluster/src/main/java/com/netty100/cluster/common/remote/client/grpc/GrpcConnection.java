...
import com.netty100.cluster.api.remote.RpcScheduledExecutor;
...
public class GrpcConnection extends Connection {
    ...
    @Override
    public Response request(Request request, long timeouts) throws CapException {
    ...
    @Override
    public void asyncRequest(Request request, final RequestCallBack requestCallBack) throws CapException {
        Payload grpcRequest = GrpcUtils.convert(request);
        ListenableFuture<Payload> requestFuture = grpcFutureServiceStub.request(grpcRequest);
        ...
        Futures.addCallback(requestFuture, new FutureCallback<Payload>() {
            ...
            public void onSuccess(@Nullable Payload grpcResponse) {
                ...
                if (response != null) {
                    if (response instanceof ErrorResponse) {
                        requestCallBack.onException(new CapException(response.getErrorCode(), response.getMessage()));
                    } else {
                        requestCallBack.onResponse(response);
                    }
                } else {
                    requestCallBack.onException(new CapException(ResponseCode.FAIL.getCode(), "response is null"));
                }
            }
