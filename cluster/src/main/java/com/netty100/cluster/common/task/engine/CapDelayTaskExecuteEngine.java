...
public class CapDelayTaskExecuteEngine extends AbstractCapTaskExecuteEngine<AbstractDelayTask> {
    ...
    public CapDelayTaskExecuteEngine(String name) {
        this(name, null);
    }
    
    public CapDelayTaskExecuteEngine(String name, Logger logger) {
        this(name, 32, logger, 100L);
    }
    
    public CapDelayTaskExecuteEngine(String name, Logger logger, long processInterval) {
        this(name, 32, logger, processInterval);
    }
    
    public CapDelayTaskExecuteEngine(String name, int initCapacity, Logger logger) {
        this(name, initCapacity, logger, 100L);
    }
    
    public CapDelayTaskExecuteEngine(String name, int initCapacity, Logger logger, long processInterval) {
        super(logger);
    ...
    protected void processTasks() {
        ...
        for (Object taskKey : keys) {
            ...
            CapTaskProcessor processor = getProcessor(taskKey);
            if (null == processor) {
            ...
            try {
                ...
                getEngineLog().error("Cap task execute error ", e);
                retryFailedTask(taskKey, task);
            }
        }
    }
