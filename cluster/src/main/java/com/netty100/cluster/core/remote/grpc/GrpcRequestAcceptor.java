...
import com.netty100.cluster.core.remote.RequestHandlerRegistry;
    ...
    public void request(Payload grpcRequest, StreamObserver<Payload> responseObserver) {
        ...
        if (!ApplicationUtils.isStarted()) {
            Payload payloadResponse = GrpcUtils.convert(
                    ErrorResponse.build(CapException.INVALID_SERVER_STATUS, "Server is starting,please try later."));
        ...
        if (requestHandler == null) {
            ...
            Payload payloadResponse = GrpcUtils
                    .convert(ErrorResponse.build(CapException.NO_HANDLER, "RequestHandler Not Found"));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        if (!requestValid) {
            ...
            Payload payloadResponse = GrpcUtils
                    .convert(ErrorResponse.build(CapException.UN_REGISTER, "Connection is unregistered."));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        try {
            ...
            Payload payloadResponse = GrpcUtils.convert(ErrorResponse.build(CapException.BAD_GATEWAY, e.getMessage()));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        if (parseObj == null) {
            ...
            Payload payloadResponse = GrpcUtils
                    .convert(ErrorResponse.build(CapException.BAD_GATEWAY, "Invalid request"));
            traceIfNecessary(payloadResponse, false);
            responseObserver.onNext(payloadResponse);
            responseObserver.onCompleted();
            return;
        ...
        try {
            ...
            // 处理业务请求，解析其他集群发送过来的数据，Handle的实现
