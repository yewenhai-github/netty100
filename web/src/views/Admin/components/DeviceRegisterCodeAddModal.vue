<template>
    <a-modal title="新增/修改" width="30%" :maskClosable="false" v-model:visible="modal.state">
        ...
        <a-form ref="formRef" :model="modal.form" :label-col="{style: {width: '100px'}}" :wrapperCol="{span: 16}">
            <!-- <a-form-item label="设备ID" name="deviceId" :rules="[{ required: true, message: '请填写名称'}]">
                <a-input v-model:value="modal.form.deviceId"/>
            </a-form-item> -->
            <a-form-item label="设备密码" name="devicePwd" :rules="[{ required: true, pattern: /^[0-9A-Z]{6}$/, message: '请填写6位大写字母及数字组成的密码'}]">
                <a-input v-model:value="modal.form.devicePwd"/>
            </a-form-item>
        </a-form>
    </a-modal>
</template>
