...
public interface MessageTypeService {
    ...
    default boolean doCommand(ChannelHandlerContext ctx, WhyMessage topeMsg, WhyNettyRemoting remotingClient, WhyKernelProperties kernelConfig){
        ...
        try {
            ...
            if (channelKey == null || SysUtility.isEmpty(channel)) {
                WhyMessageQueue.pushClientMessageLogQueue(ctx, LogPointCode.M33.getCode(), topeMsg, LogPointCode.M33.getMessage()+"[channelKey="+channelKey+"]");
                return false;
            }
            ChannelFuture future = channel.writeAndFlush(WhyMessageFactory.getClientTopeMessage(topeMsg).bytes());
            future.addListener((ChannelFutureListener) future1 -> {
                if (!future1.isSuccess()) {
                    WhyMessageQueue.pushClientMessageLogQueue(ctx, LogPointCode.M34.getCode(), topeMsg, LogPointCode.M34.getMessage() + "回调监听触发");
                }
            });
        } catch (Exception e) {
            e.printStackTrace();
