...
public class ClientChannelActiveSubscribe {
    ...
    public void onEvent(ClientChannelActiveData event) {
        // 处理消息
        WhyMessage whyMessage = event.getWhyMessage();
        ChannelHandlerContext ctx = event.getCtx();
        WhyKernelProperties kernelConfig = event.getKernelConfig();
        WhyNettyRemoting remotingClient = event.getRemotingClient();

        try {
            String clientId = WhyChannelUtils.getCurrentC2pChannelKey(whyMessage);
            //强制用户退出功能处理
            if(kickOutSameUserNext(clientId, ctx, whyMessage, remotingClient)){
                return;
            }
            ...
            if(!SysUtility.isEmpty(whyKernelProperties.getDomain())){
                //用户注册码与配置校验
                if(!doCommandConfigValidate(clientId, ctx, whyMessage) || !doCommandRegisterValidate(clientId, ctx, whyMessage)){
                    return;
                }
            }
            ...
            //客户端channel关系缓存
            WhyChannelUtils.c2pChannelCache(clientId, ctx, whyMessage, kernelConfig);

            //消息响应
            WhyChannelUtils.writeAndFlush(ctx, WhyMessageFactory.newMessage(whyMessage, ResponseCode.Rep101.getCodeBytes()));
            //调用服务器的ctx，将userId传递到服务器的ctx
            WhyChannelUtils.p2sWriteAndFlush(ctx, whyMessage, kernelConfig.getServerCacheChannelReTimes());

            //加载离线消息
//            consumerOfflineSingle(whyMessage);
            //统计信息收集
            if(SysUtility.isNotEmpty(WhyChannelUtils.getChannel(clientId))){
                WhyCountUtils.platform_c2p_connect_active_total.add(1);
            }
            //信息收集
            WhyConnectQueue.pushClientChannelActiveQueue(ctx, whyMessage.getFixedHeader(), clientId);
            //连接日志收集
            WhyConnectQueue.pushClientChannelLogQueue(ctx, whyMessage, LogPointCode.C01.getCode(), LogPointCode.C01.getMessage());
        } catch (Exception e) {
